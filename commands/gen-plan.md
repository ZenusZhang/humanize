...
Default language mode for the structured plan is English only. A separate Chinese-only (`_zh`) variant is written when `CHINESE_PLAN_ENABLED=true` (controlled by `.humanize/config.json`).
...
1. **Default Language Requirement**: The structured plan MUST be written in English only by default. A separate Chinese-only `_zh` variant is produced when `chinese_plan=true` in `.humanize/config.json`. Do NOT mix Chinese and English in the main plan file.
...
### Step 3: English-Only Language Rendering

Apply language rules to the updated structured plan:

1. The structured plan MUST be written entirely in English.
2. Do NOT include Chinese text, bilingual paired rendering, or `<Chinese text> / <English text>` patterns in the main plan file.
