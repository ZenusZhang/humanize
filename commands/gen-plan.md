...
2. **Load Project Config**: Read `.humanize/config.json` and extract plan output + routing display settings
...
3. If the file exists, parse it as JSON and extract these fields:
   - `chinese_plan`:
     - If `true` (boolean), set `CHINESE_PLAN_ENABLED=true`.
     - Otherwise (field absent, `false`, or any non-true value), set `CHINESE_PLAN_ENABLED=false`.
   - `gen_plan_coding_model`:
     - If it is a non-empty string, set `GEN_PLAN_CODING_MODEL` to that exact string.
     - Otherwise, fall back to `gpt-5.3-codex:xhigh`.
   - `gen_plan_analyze_model`:
     - If it is a non-empty string, set `GEN_PLAN_ANALYZE_MODEL` to that exact string.
     - Otherwise, fall back to `gpt-5.2:xhigh, non-codex`.
...
5. Resolve `GEN_PLAN_MODE` using the following priority (highest to lowest), with CLI flags taking priority over project config:
   ...
   - Default: `discussion`
6. A malformed JSON file should be reported as a warning but must NOT stop execution; fall back to `CHINESE_PLAN_ENABLED=false`, `GEN_PLAN_MODE=discussion`, `GEN_PLAN_CODING_MODEL=gpt-5.3-codex:xhigh`, and `GEN_PLAN_ANALYZE_MODEL=gpt-5.2:xhigh, non-codex`.

`CHINESE_PLAN_ENABLED` controls whether a `_zh` Chinese variant of the output file is written in Phase 8.
`GEN_PLAN_CODING_MODEL` and `GEN_PLAN_ANALYZE_MODEL` are display variables used when rendering the Task Breakdown routing text in Phase 7.
...
Before writing the final plan, replace `{{GEN_PLAN_CODING_MODEL}}` and `{{GEN_PLAN_ANALYZE_MODEL}}` with the resolved config-backed values loaded in Phase 0.5.
...
- `coding`: implemented by Codex worker (`/humanize:codex-worker`, default: `{{GEN_PLAN_CODING_MODEL}}`)
- `analyze`: executed via Codex analyzer (`/humanize:ask-codex`, default: `{{GEN_PLAN_ANALYZE_MODEL}}`)
...
### Convergence Status
- Status: `converged` or `partially_converged`
- Basis: <Why this status was reached>
