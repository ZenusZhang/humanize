...
# Ignore untracked project-local `.humanize/bitlesson.md` generated by this workflow.
# This keeps repeated loop starts functional without forcing immediate commits.
GIT_STATUS_FILTERED=$(echo "$GIT_STATUS_FILTERED" | grep -vE '^\?\? \.humanize/bitlesson\.md$' || true)
