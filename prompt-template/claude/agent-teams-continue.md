...
- **BitLesson Selector Required**: For each sub-task, run `bitlesson-selector` first (invoke via `scripts/bitlesson-select.sh` (preferred; runs `codex exec` with `gpt-5.2:high`) or the `bitlesson-selector` agent) and record selected lesson IDs (or `NONE`) before invoking the worker.
