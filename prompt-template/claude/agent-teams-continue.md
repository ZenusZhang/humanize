...
- **Worker Model Default**: Use `/humanize:codex-worker` with default `gpt-5.4:xhigh` unless there is a concrete reason to override.
...
- **BitLesson Selector Required**: For each sub-task, run `bitlesson-selector` first (invoke via `scripts/bitlesson-select.sh` (preferred; runs `codex exec -m gpt-5.4 -c model_reasoning_effort=xhigh`) or the `bitlesson-selector` agent) and record selected lesson IDs (or `NONE`) before invoking the worker.
