...
1. **Read `bitlesson.md` and run `bitlesson-selector`** for each fix task/sub-task before coding (invoke via `scripts/bitlesson-select.sh` (preferred; runs `codex exec -m gpt-5.4 -c model_reasoning_effort=xhigh`) or the `bitlesson-selector` agent)
